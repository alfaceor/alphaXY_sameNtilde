...
//  ch_n.waterBag(io.seedini, io.p0, M_PI/2.0);
  ...
  while (io.ttime < io.timeSteps){
    if (io.ttime%2 == 0)
      ch_o.MolecularDynamicsHBs(&ch_n, &hbL, &hbR);
    else
      ch_n.MolecularDynamicsHBs(&ch_o, &hbL, &hbR);
    io.ttime++;
  }
